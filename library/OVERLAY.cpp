...
OVERLAY::OVERLAY()
{
  ...
  signal(SIGINT, StopProgram);		//if SIGINT is caught StopProgam() will be called 
  configOverlay(P8_3);
  configOverlay(P8_4);
  configOverlay(P8_5);
  configOverlay(P8_6);
  ...
  libpru = pruioInstance();	//Instantiation of libpruio
  startClock();
}
...
void OVERLAY::destroyer_OVERLAY(){
  restoreOverlay(P8_3);
  restoreOverlay(P8_4);
  restoreOverlay(P8_5);
  restoreOverlay(P8_6);
